...
create table public.order_communication (
  id uuid primary key default gen_random_uuid(),
  order_id uuid not null references public.orders(id) on delete cascade,
...
create table public.tracking_check_logs (
  id uuid primary key default gen_random_uuid(),
...
create table public.tracking_check_log_items (
  id uuid primary key default gen_random_uuid(),
...
create index orders_order_name_idx on public.orders (order_name);
create index orders_customer_name_idx on public.orders (customer_name);
create index orders_shipping_city_idx on public.orders (shipping_city);
create index orders_financial_status_idx on public.orders (financial_status);
create index orders_fulfillment_status_idx on public.orders (fulfillment_status);
create index order_tracking_tracking_id_idx on public.order_tracking (tracking_id) where tracking_id is not null;
create index order_tracking_courier_name_idx on public.order_tracking (courier_name);
create index order_tracking_delivery_status_idx on public.order_tracking (delivery_status);
create index order_tracking_tracking_status_idx on public.order_tracking (tracking_status);
create index order_tracking_checked_at_idx on public.order_tracking (tracking_checked_at desc);
create index order_tracking_check_source_idx on public.order_tracking (tracking_check_source);
create index order_communication_confirm_status_idx on public.order_communication (confirm_txt_status);
create index order_communication_tracking_status_idx on public.order_communication (tracking_txt_status);
create index order_communication_review_status_idx on public.order_communication (review_txt_status);
create index order_comments_order_created_idx on public.order_comments (order_id, created_at desc);
create index sync_logs_started_at_idx on public.sync_logs (started_at desc);
create index tracking_check_logs_started_at_idx on public.tracking_check_logs (started_at desc);
create index tracking_check_logs_source_idx on public.tracking_check_logs (check_source);
create index tracking_check_log_items_log_idx on public.tracking_check_log_items (log_id, created_at);
create index tracking_check_log_items_order_idx on public.tracking_check_log_items (order_id, created_at desc);
create index tracking_check_log_items_status_idx on public.tracking_check_log_items (status);
create index app_settings_updated_at_idx on public.app_settings (updated_at desc);
...
comment on table public.shops is 'Shopify store configuration. Admin API tokens must stay in server environment variables.';
